...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if(body.type === 'payment_intent.succeeded'){
            ...
            const query = `SELECT company.payment
                            FROM company
            ...
            const query1 = `update company set payment = '${updateString}' where emailAddress = '${email}'`
            await executeQuery(query1).catch((e) => {
                return NextResponse.json({ type: "error" });
            });
        return NextResponse.json({ type: 'success', updated:email, result:updateString });
        }
        return NextResponse.json({ received: true });

    } catch (error) {
        console.error('Webhook signature verification failed:', error);
        return NextResponse.json({type:'error',msg:'Webhook signature verification failed'});
    }
}